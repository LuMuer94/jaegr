...
class MemberView implements OnInit{
  ...
  startAddingUser()
  {
    if( user!=null && group!=null && user.equals(group.admin) ){
  ...
  Future addUser( User user ) async {
    User u = await  restService.getCurrentUser();
    if ( u.equals( group.admin)) {
  ...
  Future deleteUser( User user ) async {
    User u = await restService.getCurrentUser();
    if ( u.equals(group.admin) && !user.equals(group.admin) ){
      await restService.removeUserFromGroup(group.id, user.id);
      group = await restService.getGroupById(group.id);
      getMembers();
